...
export class UpdateService {
  ...
  public async getCurrentVersion(): Promise<string> {
    ...
    try {
      ...
      this.logger.error('Error reading current version:', errorMessage);
  ...
  public async getLatestVersion(): Promise<string> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error('Error fetching latest version:', errorMessage);
      throw new Error(`Failed to get latest version from npm: ${errorMessage}`);
    }
  }
  ...
  public async isUpdateAvailable(): Promise<boolean> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error('Error checking if update is available:', errorMessage);
      return false;
    }
  }
  ...
  private async getLastUpdateCheckTimestamp(): Promise<number> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `Warning: Could not read last update check timestamp. Assuming check is needed. Error: ${errorMessage}`
      );
      return 0;
    }
  }
   ...
   */
  public async performUpdate(): Promise<{
    success: boolean;
    error?: Error;
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`❌ Could not check for updates: ${errorMessage}`);
    ...
    try {
      const { stderr } = await execAsync(kNpmInstallCommand);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`❌ Error updating ${kPackageName}: ${errorMessage}`, error);
      return { success: false, error: error instanceof Error ? error : new Error(String(error)) };
    }
  }
  ...
  private async setLastUpdateCheckTimestamp(): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `Warning: Could not set last update check timestamp. Error: ${errorMessage}`
      );
    }
  }
  ...
  public async runUpdateCheckAndNotify(): Promise<void> {
    try {
      ...
      try {
        ...
        this.logger.warn(`⚠️ Could not check for ${kPackageName} updates: ${errorMessage}`);
        return;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `⚠️ An unexpected error occurred during the update check process: ${errorMessage}`
      );
    }
  }
}
